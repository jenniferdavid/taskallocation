    ...
    SubPub()
        {
            ...
            deltaMatPub_ = nh_.advertise<std_msgs::Float64MultiArray>("/deltamat",10);
            
            navi_msgs::Problem input;
            boost::shared_ptr<navi_msgs::Problem const> inputptr;
            int totalModels;
            std::vector<std::pair<double,double> > totalCoord;
            inputptr = ros::topic::waitForMessage<navi_msgs::Problem>("/problem",ros::Duration(10));
            if (inputptr == NULL)
            {ROS_INFO("No problem input messages received");}
            else
                {
                    input = * inputptr;
                    totalModels = input.nTotalmodels;
                    
                    navi_msgs::ItemStruct data;
                    boost::shared_ptr<navi_msgs::ItemStruct const> dataptr;
                    dataptr = ros::topic::waitForMessage<navi_msgs::ItemStruct>("/coords",ros::Duration(10));
                    if (dataptr == NULL)
                        {ROS_INFO("No coords input messages received");}
                    else
                        {
                            data = * dataptr;
                            for (int i=0; i< totalModels; ++i) //msg->coords.size()
                                {
                                    ROS_INFO_STREAM("Model ID: " << data.coords[i].id << " x: " << data.coords[i].x << " y: " << data.coords[i].y << " yaw: " << data.coords[i].yaw);
                                    totalCoord.push_back(std::make_pair(data.coords[i].x, data.coords[i].y));
                                }
                        }
